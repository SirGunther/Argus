...
Filesystem authority is declared as a **named host-neutral scope** (`session-root`), never as a host path, so a manifest cannot express a traversal, an absolute path, or a drive letter, and a future container or native host can map the same scope onto its own mount without changing the manifest.
