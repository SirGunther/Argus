...
| --- | --- | --- |
| `filesystem` | `{ read: ["session-root"], write: ["session-root"] }` | no scopes |
...
Filesystem authority is declared as the **named scope** `session-root`, never as a host path. A manifest therefore cannot express a traversal, an absolute path, or a drive letter, and a future container or native host can map the same scope name onto its own mount without the manifest changing. `session-root` additionally requires `ARGUS_SESSION_ROOT` in the runtime environment allowlist, because a component that cannot learn which root it holds cannot meaningfully hold it.
...
| Every other component (16 of 23) | none |
...
| --- | --- | --- |
| `filesystem.read` | **Node-runtime-enforced** | `--allow-fs-read` limited to the component directory, the shared component libraries, and a granted session root |
...
| --- | --- | --- | --- |
| focused | `node --test tests/phase8-permissions-packaging.test.mjs` | Phase 8 permissions and packaging | pass — 20/20 |
