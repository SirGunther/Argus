...
- Introduced the governed carrier that transmits one settled `scribe_batch_evaluated` artifact from the extraction boundary back to the Scribe coordinator. The payload carries required `batch_attempt` plus a required `batch` object referencing `argus.scribe-batch-evaluated.v1` by `$ref`; the artifact shape is not redefined here.
...
- SCRIBE-04B reconciliation, before the first production merge of this version, requires message `batch_attempt` to equal artifact `batch.attempt`. Both mean coordinator batch attempt and never provider completion attempt.
