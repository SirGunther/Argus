...
The real Whisper adapter buffers bounded PCM without inference during `audio.chunk` handling and runs one authoritative inference for each accepted `audio.flush` from pause, Stop, or Close. During an active utterance, the desktop host sends at most one coalesced immutable `audio.preview` snapshot roughly every 2 seconds to the same serial Whisper lane. The adapter emits only the existing revisioned `transcript.partial` projection for a non-empty real Whisper result; final pause/rollover work cancels or outranks provisional work. Because the installed whisper.cpp runtime is batch-oriented, observed preview latency is approximately the 2-second cadence plus one local Whisper invocation, rather than word-by-word streaming.

Both provisional and final results reconstruct lexical words deterministically from raw provider-token whitespace: BPE continuations join, contractions and trailing punctuation attach, and no model guesses spacing. For a word made from multiple provider tokens, the committed confidence is the minimum token probability, a conservative score that cannot overstate any contributing token. Whisper control/timestamp tokens and known non-speech markers are removed before output; marker-only final windows emit `transcript.empty`, while token-derived timestamps and audio-chunk provenance are persisted. A startup recovery pass routes any session left `recording` by an unclean exit through the lifecycle owner into `stopped`, so the user must explicitly Resume it. Abandoned `.argus-stt` WAV/JSON artifacts are removed only when their process ownership is no longer live.
...
The architecture decision record in `Architecture/DesignDecisions.md` explains why extraction and optional classification are separate operations, how idle-time enrichment should work, and which transcript context belongs in each payload. [`Architecture/OperationalAgentRoles.md`](Architecture/OperationalAgentRoles.md) defines Scribe as the current recording role and reserves Assistant and Actor as future boundaries without adding them to present implementation scope. The isolated implementation tickets, dependency waves, and merge order are in [`docs/plans/SCRIBE-PIPELINE-INTEGRATION-TODO.md`](docs/plans/SCRIBE-PIPELINE-INTEGRATION-TODO.md); each ticket is dispatched with [`docs/plans/ARGUS-ISOLATED-TICKET-HANDOFF.md`](docs/plans/ARGUS-ISOLATED-TICKET-HANDOFF.md). Real-runtime acceptance evidence for that pipeline, and the user validation steps that remain, are in [`docs/validation/SCRIBE-ACCEPTANCE-VALIDATION.md`](docs/validation/SCRIBE-ACCEPTANCE-VALIDATION.md). A previously open defect at the session-start seam - a duplicate policy publication producing a conflicting recovery request - is resolved (`SCRIBE-06B`); the record is kept for history at [`docs/incidents/2026-09-12-scribe-session-start-recovery-conflict.md`](docs/incidents/2026-09-12-scribe-session-start-recovery-conflict.md).
