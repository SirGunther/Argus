...
The real Whisper adapter buffers bounded PCM without inference during `audio.chunk` handling and runs one inference only for each accepted `audio.flush` from pause, Stop, or Close. It reconstructs lexical words deterministically from raw provider-token whitespace: BPE continuations join, contractions and trailing punctuation attach, and no model guesses spacing. For a word made from multiple provider tokens, the committed confidence is the minimum token probability, a conservative score that cannot overstate any contributing token. Whisper control/timestamp tokens and known non-speech markers are removed before committed words; marker-only windows emit `transcript.empty`, while token-derived timestamps and audio-chunk provenance are persisted. A startup recovery pass routes any session left `recording` by an unclean exit through the lifecycle owner into `stopped`, so the user must explicitly Resume it. Abandoned `.argus-stt` WAV/JSON artifacts are removed only when their process ownership is no longer live.
