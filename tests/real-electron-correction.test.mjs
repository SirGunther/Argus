...
test('Stop and Close complete after a finalization failure', async () => {
  for (const command of ['session.stop', 'session.close']) {
    ...
    application.graph = {
      closed: false,
      async dispatchFrom(_from, plane, type, sessionId, payload) {
        ...
        if (type === 'scribe.session-closing') {
          application.handleGraphMessage({
            ...
            payload: { session_id: sessionId, request_id: payload.request_id, flushed_at: new Date().toISOString(), accepted: true, admitted_through: { last_segment_id: null, last_sequence: -1, last_revision: 0 }, pending_rows: 0 }
          });
        }
      },
