...
test('fake transcript source independently emits valid transcript segments', async () => {
  ...
  const result = await runService(path.join(root, 'services/fake-transcript-source/service.json'), [start], 4);
  const segments = result.outputs.filter((message) => message.message_type === 'transcript.segment');
...
test('window selector independently emits a valid provenance-bearing context window', async () => {
  ...
  const result = await runService(path.join(root, 'services/transcript-window-selector/service.json'), inputs, 4);
  const window = result.outputs.find((message) => message.message_type === 'transcript.context-window');
  assert.deepEqual(registry.validateEnvelope(window), []);
  assert.equal(window.payload.segments.length, 3);
  assert.equal(window.payload.source.first_segment_id, 'segment-1');
  assert.equal(window.payload.source.last_segment_id, 'segment-3');
});
...
for (const implementation of ['log-extractor-concise', 'log-extractor-passthrough']) {
  test(`${implementation} independently honors the logged-item draft contract`, async () => {
    ...
    const result = await runService(path.join(root, `services/${implementation}/service.json`), [input], 2);
    const draft = result.outputs.find((message) => message.message_type === 'logged-item.draft');
    assert.deepEqual(registry.validateEnvelope(draft), []);
    assert.equal(draft.payload.source.first_segment_id, 'segment-1');
    assert.ok(draft.payload.text.length > 0);
  });
}
...
test('memory store independently emits the stored-item contract', async () => {
  ...
  const result = await runService(path.join(root, 'services/logged-item-memory-store/service.json'), [draft], 2);
  const stored = result.outputs.find((message) => message.message_type === 'logged-item.stored');
  assert.deepEqual(registry.validateEnvelope(stored), []);
  assert.equal(stored.payload.item_id, 'log-contract-test');
});
