...
test('valid graph resolves four isolated services, six runtime components, and explicit two-plane wires', async () => {
  const prepared = await validateGraphFile(graphFile);
  assert.equal(prepared.services.size, 4);
  assert.equal(prepared.endpoints.size, 10);
  assert.equal(prepared.definition.domain_wires.length, 4);
  assert.equal(prepared.definition.control_wires.length, 31);
});
...
test('removing a rejection wire makes nonfatal stale-result handling impossible', async () => {
  const { definition, graphFile: absoluteGraphFile } = await loadGraphDefinition(graphFile);
  const invalid = structuredClone(definition);
  invalid.control_wires = invalid.control_wires.filter((wire) => !(wire.from === 'logged-item-store' && wire.contract === 'operation.rejected'));
  await assert.rejects(() => prepareGraph(invalid, absoluteGraphFile), /logged-item-store requires an explicit operation\.rejected control wire/);
});
