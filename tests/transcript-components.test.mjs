...
test('meaningful unresolved correction becomes a focused review flag instead of rewriting text', async () => {
  ...
  const result = await runService(manifest('active-transcript-owner'), [...await sttDomainOutputs(), resolution], 12);
...
test('active owner automatically accepts eligible correction, finalizes formatting, revises optimistically, and preserves every history revision', async () => {
  ...
  const result = await runService(manifest('active-transcript-owner'), [...sttInputs, resolution, update, stale], 16);
...
test('permanent history is append-only, replay-safe, revision-complete, and rejects conflicting reuse', async () => {
  ...
  const conflict = createEnvelope({ plane: 'domain', messageType: 'transcript.history-append', producer: 'test', correlationId: sessionId, schemaVersion: '1.3.0', idempotencyKey: 'conflicting-history-entry', payload: {
...
test('Phase 4C graph exposes working-document results and default-deny wiring excludes partials from history', async () => {
  ...
  assert.deepEqual(result.completions.map((message) => message.message_type), ['transcript.segment', 'transcript.segment-stored', 'transcript.history-appended']);
  assert.equal(result.completions[0].payload.text, 'Argus, you ready?');
...
async function finalizedActiveOutputs(includeUpdate = false) {
  const inputs = [...await sttDomainOutputs(), correctionResolution()];
  ...
  const result = await runService(manifest('active-transcript-owner'), inputs, includeUpdate ? 15 : 12);
...
function correctionResolution(confidence = 0.96) {
  ...
  return createEnvelope({ plane: 'domain', messageType: 'transcript.correction-resolved', producer: 'test', correlationId: sessionId, idempotencyKey: 'correction-resolution', payload: {
    request_id: `${sessionId}-utterance-0-boundary-correction`, session_id: sessionId, utterance_id: `${sessionId}-utterance-0`, boundary_id: `${sessionId}-utterance-0-boundary`,
    proposals: [{ proposal_id: `${sessionId}-proposal-0`, target_word_id: first, target_word_sequence: 0, expected_text: 'are', proposed_text: 'Argus', confidence, basis: 'acoustic-and-contextual', context: { first_word_id: first, last_word_id: `${sessionId}-word-2` } }],
