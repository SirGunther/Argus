...
test('compact Electron viewport contract keeps primary controls visible and secondary details collapsed', async () => {
  ...
  assert.match(css, /#transcriptList \.data-row:not\(\.live-preview\) > \.row-main, #derivedList \.data-row > \.row-main \{\s*margin-left:\s*4px;\s*\}/, 'finalized transcript rows and logged items must both keep a 4px timestamp/text inset');
