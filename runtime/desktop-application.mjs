...
export class DesktopApplication {
  constructor({ root, graphFile, sessionRoot, environment = process.env, providerSettingsStore, credentialStore, diagnosticsEnabled = false, diagnosticsOutput, diagnosticClock, diagnosticStallThresholdMs = PIPELINE_STALL_THRESHOLD_MS } = {}) {
    ...
    this.transcriptProjectedRevisions = new Set();
  ...
  async executeCommand(payload) {
    ...
    switch (payload.command) {
      ...
      case 'transcript.edit':
        if (!this.transcript.some((item) => item.segment_id === payload.segment_id && !item.provisional)) throw Object.assign(new Error(`Unknown finalized transcript segment ${payload.segment_id}.`), { code: 'TRANSCRIPT_SEGMENT_NOT_FOUND' });
        if (this.metadata?.state === 'closed') throw Object.assign(new Error('Closed-session transcript rows are read-only.'), { code: 'SESSION_CLOSED' });
  ...
  projectFinalTranscriptSegment(payload) {
    ...
    this.diagnostics.log('ui.transcript-row-projected', { session_id: row.session_id, utterance_id: utteranceId, projection_id: projection.message_id, segment_id: row.segment_id, revision: row.revision, provisional: false });
    if (utteranceId) {
  ...
  transcriptRow(item, utteranceId, sessionState) { return { session_id: item.session_id, ...(utteranceId ? { utterance_id: utteranceId } : {}), segment_id: item.segment_id, revision: item.revision || 0, sequence: item.sequence, start_time: item.start_time, end_time: item.end_time, text: item.text, provisional: false, read_only: Boolean(item.read_only) || sessionState === 'closed', review_flags: item.review_flags || [] }; }
