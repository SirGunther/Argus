...
**Status:** Open. Confirmed on `origin/main` at `5dbeae3`. No fix in this branch — `SCRIBE-06` owns no production file.
...
**Severity:** Not established. See *Impact* below — an earlier revision of this document asserted a
product-breaking severity that was withdrawn under review.
...
The assertion is carried in `tests/scribe-real-acceptance.test.mjs` as a `todo`: the duplicate
emission is certain, the fix is outside this ticket's ownership, and a red gate would misreport the
rest of the baseline. It needs no model provider.

## Required ownership revision

The duplicate emission is in production code that `SCRIBE-06` does not own. The files that could
carry a repair are `services/scribe-coordinator/index.mjs` (recovery-request identity),
`services/scribe-policy-source/index.mjs` (republication of an unchanged policy identity), and
`runtime/desktop-application.mjs` (publishing twice at session start). Which of those is correct is
the owning ticket's decision, not this one's.
