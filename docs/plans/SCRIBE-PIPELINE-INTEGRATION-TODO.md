...
8. Every implementation agent must run `C:\dustin-thomason\scripts\notify-agent-complete.ps1` after pushing and before reporting completion, using a 5–9 word message containing `<your-model-name>`, e.g., Claude, Codex, etc.. If blocked and user input is required, the agent must send the notification before asking the question.
...
* [ ] Send a 5–9 word notification containing `<your-model-name>`, stating that SCRIBE-05 implementation is ready for verification, then pause for the lower-tier verification stage.
...
* [ ] Send a 5–9 word notification containing `<your-model-name>`, stating that SCRIBE-06 preparation is complete, then pause for the high-reasoning acceptance stage.
...
* [ ] Send a 5–9 word notification containing `<your-model-name>`, stating that SCRIBE-06 acceptance analysis is complete, then pause for the lower-tier closure stage.
...
SCRIBE-05 is complete: the reconciled Scribe foundation is joined into the production graph and desktop lifecycle, all three stages verified, and the branch pushed. Assign **SCRIBE-06** next using `docs/plans/ARGUS-ISOLATED-TICKET-HANDOFF.md` plus the complete SCRIBE-06 ticket above, starting from the current `origin/main` once SCRIBE-05 is merged.
